...
implements JGEngineInterface  {
	...
	Paint imagepainter = new Paint(); // fancy images, includes blend mode
	Paint simpleimagepainter = new Paint(); // simple images
	Bitmap background=null;
	Canvas bgg=null;
	Paint bgpainter = new Paint();
	Paint clearpaint = new Paint(); // for clearing areas only
	...
	void initActivity(JGActivity act) {
		currentact = act;
		multitouchsupported = act.getPackageManager().hasSystemFeature(
	...
	class JGView extends View implements SensorEventListener {
		...
		@Override protected void onDraw(Canvas g) { try {
			...
			synchronized (el.objects) {
				// in extended mode, first draw bg_images to buffer, before
				// copying background to buffer.
				if (el.bg_extended_mode) {
					drawParallaxBG();
				}
		...
		private void drawParallaxBG() {
			for (int i=el.bg_images.size()-1; i>=0; i--) {
				...
				AndroidImage bg_image_i = (AndroidImage)
					el.getImage(bg_image.imgname);
				JGPoint size = bg_image_i.getSize();
				// find x,y,w,h on physical canvas
				int w = size.x;
				int h = size.y;
				int x = el.scaleXPos(-bg_image.xofs,true);
				int y = el.scaleYPos(-bg_image.yofs,true);
				// determine start (x,y) and end (endx,endy) for tiling
				int endx=w, endy=h;
				// change x to number < 0
				if (bg_image.wrapx) {
					endx = el.width;
					x = el.moduloFloor(x,w);
					if (x>0) x -= w;
				}
				if (bg_image.wrapy) {
					endy = el.height;
					y = el.moduloFloor(y,h);
					if (y>0) y -= h;
				}
				x += el.canvas_xofs;
				y += el.canvas_yofs;
				// we tile manually until we find a more efficient method
				// XXX do we use el.smooth_magnify already?
				for (int dy=y; dy<endy; dy += h) {
					for (int dx=x; dx<endx; dx += w) {
						bufg.drawBitmap(bg_image_i.img,dx,dy,
							simpleimagepainter);
					}
				}
			}
		...
		public boolean onTouchEvent(MotionEvent me) {
			...
			double x = me.getX();
			double y = me.getY();
			// Get the last changed x and y. We can do that only by going
			// through the history of positions and checking which position was
			// changed last. 
			// XXX this doesn't work and may not be reliable across different
			// devices, a lot of problems are reported with the history
			// functions.
			// We will likely need to implement full multitouch handling to
			// get the last changed x and y
			/*if (action == MotionEvent.ACTION_MOVE) {
				// If there is a history of more than 1 element,
				// we find out which pointer was changed last.
				// We start at the most recent history and find a pointer 
				// that changed position.
				for (int h=me.getHistorySize()-1; h>0; h--) {
					for (int j=0; j<me.getPointerCount(); j++) {
						double thisx = me.getHistoricalX(j,h);
						double thisy = me.getHistoricalY(j,h);
						if (thisx != me.getHistoricalX(j,h-1)
						||  thisy != me.getHistoricalY(j,h-1) ) {
							x = thisx;
							y = thisy;
							break; // self terminating search
						}
					}
				}
			}*/
			...
			// multitouch
			if (press||release) {
				// press and release represents events on the primary pointer
				int ptrid = me.getPointerId(ptridx);
				// find match with pointer id and find free spot
				int firstfree=-1;
				int arridx=-1;
				for (int i=0; i<androidTouchIDs.length; i++) {
					if (androidTouchIDs[i]==-1) {
						firstfree = i;
					} else if (androidTouchIDs[i]==ptrid) {
						arridx = i;
					}
				}
				if (press && arridx==-1 && firstfree!=-1) {
					// new press, free slot available
					androidTouchIDs[firstfree] = ptrid;
					jgameTouchIDs[firstfree] = nextTouchID++;
				}
				if (release && arridx!=-1) {
					// release of known id
					androidTouchIDs[arridx] = -1;
					androidTouchX[arridx] = -1;
					androidTouchY[arridx] = -1;
					jgameTouchIDs[arridx] = -1;
				}
			}
			// handle positions
			int pointerCount = me.getPointerCount();
			for (int p = 0; p < pointerCount; p++) {
				int ptrid = me.getPointerId(p);
				int arridx=-1;
				for (int i=0; i<androidTouchIDs.length; i++) {
					if (androidTouchIDs[i]==ptrid) {
						arridx = i;
						break;
					}
				}
				if (arridx!=-1) {
					androidTouchX[arridx] =
						(me.getX(p)-el.canvas_xofs)/el.x_scale_fac;
					androidTouchY[arridx] =
						(me.getY(p)-el.canvas_yofs)/el.y_scale_fac;
				}
			}
			return true;
		}
	...
	public void setBGImage(String bgimg) {
		el.setBGImage(bgimg); // stay in basic mode
	}

	public void setBGImage(int depth, String bgimg,boolean wrapx,boolean wrapy){
		el.setBGImage(bgimg,depth,wrapx,wrapy,true);
	}
	...
	public void drawTile(int xi,int yi,int tileid) {
		...
		if (el.bg_extended_mode) {
			// parallax bg: drawn separately. Clear tile area only.
	...
	boolean centered) {
		el.setBGImgOffset(depth,xofs,yofs,centered);
	}
	...
	* passing a null means ignore that argument. */
	...
	public void setBlendMode(int src_func, int dst_func) {
		if (src_func==1 && dst_func==-1) { // default
			//imagepainter.setXfermode(null);
		} else if (src_func==1 && dst_func==0) { // neon
			// ADD is somehow not defined
			//imagepainter.setXfermode(new PorterDuffXfermode(
			//	PorterDuff.Mode.ADD));
		}
	}
	...
	/** List of current Android touch IDs, -1 indicates ID is not used / was
	 * released. */
	int [] androidTouchIDs = new int[] {-1,-1,-1,-1, -1,-1,-1,-1};
	/** Last positions corresponding to the touch IDs */
	double [] androidTouchX   = new double[] {-1,-1,-1,-1, -1,-1,-1,-1};
	double [] androidTouchY   = new double[] {-1,-1,-1,-1, -1,-1,-1,-1};
	int nextTouchID=1;
	/** Mapping of Android array indexes to JGame touch IDs. */
	int [] jgameTouchIDs   = new int[] {-1,-1,-1,-1, -1,-1,-1,-1};
	/** is defined when JGActivity is created. */
	...
	public int [] touchGetIDs() {
		int nr=0;
		for (int i=0; i<jgameTouchIDs.length; i++) {
			if (jgameTouchIDs[i]>=0) {
				// ignore extreme values to work around bug in 
				// arnova devices
				if (androidTouchX[i] < -200
				||  androidTouchY[i] < -200
				||  androidTouchX[i] > 3000
				||  androidTouchY[i] > 3000) continue;
				nr++;
			}
		}
		int idx=0;
	...
	public double touchGetX(int id) {
		for (int i=0; i<jgameTouchIDs.length; i++) {
			if (jgameTouchIDs[i]==id) return androidTouchX[i];
		}
		return 0;
	...
	public double touchGetY(int id) {
		for (int i=0; i<jgameTouchIDs.length; i++) {
			if (jgameTouchIDs[i]==id) return androidTouchY[i];
		}
		return 0;
	}
	public boolean touchIDExists(int id) {
		for (int i=0; i<jgameTouchIDs.length; i++) {
			if (jgameTouchIDs[i]==id) return true;
		}
		return false;
	}
