...
class MIDPImage implements JGImage {
	...
	int[] imgdata, int imgdataofs,int linewidth) {
		int [] imgdataflat = imgdata;
		if (imgdataofs!=0 || linewidth!=imgwidth) {
			// convert data to "flat" data
			imgdataflat =new int[imgwidth*imgheight];
			for (int y=0; y<imgheight; y++) {
				System.arraycopy(
					/* src */ imgdata, imgdataofs + linewidth*y,
					/* dst */ imgdataflat, imgwidth*y,
					/* length*/ imgwidth);
			}
		}
		return new MIDPImage(Image.createRGBImage(
			imgdataflat,imgwidth,imgheight,true));
	}
