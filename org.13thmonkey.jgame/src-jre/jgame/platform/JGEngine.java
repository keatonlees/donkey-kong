...
public abstract class JGEngine extends Applet implements JGEngineInterface {
	...
	protected final MultiDimensionStatus accelGyroStatus = new MultiDimensionStatus("accelAngles", "accelAccelerations",
			"gyroAngles", "gyroAngularSpeeds", "filteredAngles");
	...
	public double getAccelX() {
		return accelGyroStatus.getData()[1].x;
	}

	@Override
	public double getAccelY() {
		return accelGyroStatus.getData()[1].y;
	}

	@Override
	public double getAccelZ() {
		return accelGyroStatus.getData()[1].z;
	}

	@Override
	public double[] getAccelVec() {
		Point3d accel = accelGyroStatus.getData()[1];
		return new double[] { accel.x, accel.y, accel.z };
	}

	@Override
	public double getGyroAngleX() {
		return accelGyroStatus.getData()[2].x;
	}

	@Override
	public double getGyroAngleY() {
		return accelGyroStatus.getData()[2].y;
	}

	@Override
	public double getGyroAngleZ() {
		return accelGyroStatus.getData()[2].z;
	}

	@Override
	public double getGyroX() {
		return accelGyroStatus.getData()[3].x;
	}

	@Override
	public double getGyroY() {
		return accelGyroStatus.getData()[3].y;
	}

	@Override
	public double getGyroZ() {
		return accelGyroStatus.getData()[3].z;
	}
	...
	public double[] getGyroVec() {
		Point3d gyro = accelGyroStatus.getData()[3];
		return new double[] { gyro.x, gyro.y, gyro.z };
	}

	private UdpServerThread udpReceiver;

	public void startAccelGyroFromUdp(int port) {
		startAccelGyroFromUdp(port, null);
	}

	public void startAccelGyroFromUdp(int port, IUdpServerListener... listeners) {
		if (udpReceiver == null || !udpReceiver.isRunning()) {
			udpReceiver = new UdpServerThread(port, accelGyroStatus);
			...
			if (listeners != null) {
				for (IUdpServerListener l : listeners) {
					udpReceiver.add(l);
				}
			}
			udpReceiver.start();
	...
	public void startAccelGyroNative() {
		nativeGyro = new GyroDataUpdater(accelGyroStatus);
		nativeGyro.start();
