...
public class JGObject {
	...
	/**
	 * The engine's viewWidth and viewHeight, stored in a local variable for
	 * extra speed.
	 */
	public static int viewwidth, viewheight;
	/**
	 * The engine's tileWidth and tileHeight, stored in a local variable for
	 * extra speed.
	 */
	...
	/**
	 * The engine's gamespeed setting, stored in a local variable for extra
	 * speed.
	 */
	public static double gamespeed;
	/**
	 * The engine's pfWidth/pfHeight, stored in a local variable for extra
	 * speed.
	 */
	...
	String animid = null;
	/**
	 * cached value: has to be recomputed when image changes; simply set to null
	 * to do this.
	 */
	...
	/**
	 * You can use this to call methods in the object's engine. Even handier is
	 * to have the objects as inner class of the engine.
	 */
	 ...
	 * with the given ID, and setImage if found. Passing null clears the image
	...
	/**
	 * Set ID of image to display; clear animation. Passing null clears the
	 * image.
	 */
	...
	/**
	 * Set the animation to the given default animation definition, or leave it
	 * as it was if the anim_id is unchanged. Subsequent changes made in the
	 * animation's parameters do not change the default animation definition.
	 * The changes will be preserved if another call to setAnimation is made
	 * with the same anim_id. If you want to reset the animation to the original
	 * settings, use resetAnimation().
	 */
	 ...
	 * 
	 * @param unique_id
	 *            append name with unique ID if unique_id set
	 * @param gfxname
	 *            id of animation or image, null = no image
	 */
	...
	/**
	 * Create object with given tile bbox, old style. Old style constructors are
	 * not compatible with the JGame Flash parameter order.
	 * 
	 * @param unique_id
	 *            append name with unique ID if unique_id set
	 * @param gfxname
	...
	/**
	 * Create object with given absolute speed, old style. Old style
	 * constructors are not compatible with the JGame Flash parameter order.
	 * 
	 * @param unique_id
	 *            append name with unique ID if unique_id set
	...
	/**
	 * Create object with given tile bbox and absolute speed, old style. Old
	 * style constructors are not compatible with the JGame Flash parameter
	 * order.
	 * 
	 * @param unique_id
	 *            append name with unique ID if unique_id set
	 ...
	 * Old style constructors are not compatible with the JGame Flash parameter
	 * order.
	 * 
	 * @param unique_id
	 *            append name with unique ID if unique_id set
	...
	/**
	 * Get object collision bounding box in pixels. Has actual coordinate
	 * offset.
	 * 
	...
	/**
	 * Get object collision bounding box in pixels. Has actual coordinate
	 * offset. Uses a fixed temp variable to store the bbox, so no object is
	 * created. The returned object may not be modified, nor its contents used
	 * after the method is re-entered.
	 * 
	...
	/**
	 * Get tile collision bounding box in pixels. Bounding box has actual
	 * coordinate offset.
	 * 
	 * @return copy of bbox in pixel coordinates, null if no bbox
	 */
	...
	/**
	 * Get tile collision bounding box in pixels. Bounding box has actual
	 * coordinate offset. Returns reference to a fixed internal variable. Do not
	 * change the object returned, avoid re-entering this method and then using
	 ...
	 * (xspeed*gamespeed-epsilon, yspeed*gamespeed-epsilon), corresponding to
	 * the default is...Aligned margin.
	 */
	 ...
	 * 
	 * @param gridsnapx
	 *            margin below which to snap, 0.0 is no snap
	 * @param gridsnapy
	 *            margin below which to snap, 0.0 is no snap
	 */
	...
	// }
	/**
	 * Snap an object's tile bbox corner to grid; floats are rounded down. Snaps
	 * to bottom or right of object instead of top and left if the resp. flags
	 * are set. Note that bottom and right alignment means that the object's
	 * bounding box is one pixel away from crossing the tile border.
	 * 
	 * @param snap_right
	 *            snap the right hand side of the tile bbox
	 * @param snap_bottom
	 *            snap the bottom of the tile bbox
	 */
	...
	/**
	 * Get the tile index coordinates of all the tiles that the object's tile
	 * bbox overlaps with. Always returns the same temp object, so no object
	...
	/**
	 * Get the tile indices spanning the tiles that the object has the most
	 * overlap with. The size of the span is always the same as size of the tile
	 * bbox in tiles. For example, if the tile bbox is 48x32 pixels and the tile
	 * size is 32x32 pixels, the size in tiles is always 1.5x1 tiles, which is
	 * rounded up to 2x1 tiles. Always returns the same temp object, so no
	...
	/**
	 * Get the tile indices spanning the tiles that the object's last bounding
	 * box had the most overlap with. Always returns the same temp object, so no
	 * object creation is necessary.
	 * 
	 * @return tile index coordinates, null if no tile bbox
	 */
	 ...
	 * tile. If the object is larger, you get the top left tile of the center
	 * tiles. Always returns the same temp object, so no object creation is
	 * necessary.
	 * 
	...
	/**
	 * Get the topleftmost tile of the object. Always returns the same temp
	 * object, so no object creation is necessary.
	 * 
	 ...
	 * return false when the object is snapped to grid, and then moves
	 * xspeed*gamespeed away from its aligned position.
	 */
	 ...
	 * always return true when the object crosses the tile alignment line, and
	 * return false when the object is snapped to grid, and then moves
	 * yspeed*gamespeed away from its aligned position.
	 */
	 ...
	 * added to the margin, so that isXAligned(1.0000, 1.0000) always returns
	 * true.
	 */
	 ...
	 * 
	 * @param cid
	 *            cid mask of objects to consider, 0=any
	 */
	 ...
	 * Engine when I define this one in Object public int checkBGCollision() {
	 ...
	 * tile bounding box is used as the object's size, if there is none, we use
	 * a zero-size bounding box. isOnScreen is equal to isOnPF, but is
	 * deprecated because the name is ambiguous.
	 * 
	 * @deprecated Use isOnPF and isInView according to your situation.
	 * @param marginx
	 *            pixel margin, may be negative
	 * @param marginy
	 *            pixel margin, may be negative
	 */
	@Deprecated
	...
	/**
	 * Margin is the margin beyond which the object is considered off-view. The
	 * tile bounding box is used as the object's size, if there is none, we use
	 ...
	 * playfield. The tile bounding box is used as the object's size, if there
	 * is none, we use a zero-size bounding box.
	 * 
	 * @param marginx
	 *            pixel margin, may be negative
	 * @param marginy
	...
	/**
	 * A Boolean AND shorthand to use for collision; returns (value&amp;mask) !=
	 * 0.
	 */
	 ...
	 * implementation once for every moveObjects; normally it is not necessary
	 * to call this.
	 */
	...
	/**
	 * Signal that a new frame has just been updated; make snapshot of object
	 * state. Should only be called by the JGEngine implementation.
	 */
	 ...
	 * performed by the JGEngine implementation after each moveObjects. Normally
	 * it is not necessary to call this, but it may be in special cases.
	 */
	...
	/**
	 * Suspend object until either resume is called or, if resume_in_view is
	 * true, when it comes into view. A suspended object does not participate in
	 * any move, hit, or paint.
	 */
	public void suspend() {
		is_suspended = true;
	}
	...
	/**
	 * Signal to object that remove is done, don't call directly. This is used
	 * by the engine to signal that the object should be finalised.
	 */
	 ...
	 * removed at the end of this moveObjects, checkCollision, or
	 * checkBGCollision action, or immediately if not inside either of these
	 * actions.
	 */
	public void remove() {
		if (isAlive())
	...
	/**
	 * Override to handle tile collision; default is do nothing. This method is
	 * always called when hit_bg(int) is also called, only it may be called
	 * multiple times, namely once for each tile the object collides with. The
	 * arguments pass the tile cid and position of that tile. It is always
	 * called after hit_bg(int) and hit_bg(int,int,int,int,int) have been
	 * called.
	 */
