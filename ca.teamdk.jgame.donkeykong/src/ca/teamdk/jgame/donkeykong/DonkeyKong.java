...
public class DonkeyKong extends StdGame {
	...
	public void initGame() {
		...
		defineImage(JumpMan.TILE + "c", JumpMan.TILE + "c", JumpMan.COLLISION_ID, "jumpman_climb.png", "-");
		defineImage(JumpMan.TILE + "c2", JumpMan.TILE + "c2", JumpMan.COLLISION_ID, "jumpman_climb.png", "x");
		defineImage(JumpMan.TILE + "u", JumpMan.TILE + "u", JumpMan.COLLISION_ID, "jumpman_up.png", "-");
		...
		defineAnimation("jac", climbUp, 0.2);

		// PAULINE
		defineImage(Pauline.TILE, Pauline.TILE, Pauline.COLLISION_ID, "pauline2.png", "-");
		defineImage(Pauline.TILE + "1", Pauline.TILE + "1", Pauline.COLLISION_ID, "pauline1.png", "-");
		defineImage(Pauline.TILE + "3", Pauline.TILE + "3", Pauline.COLLISION_ID, "pauline3.png", "-");

		defineAnimation("pa", paulineAnim, 0.05);
		...
		defineImage(Heart.TILE, Heart.TILE, Heart.COLLISION_ID, "heart2.png", "-");
		
		defineAudioClip("st", "startingTrack.wav");
		defineAudioClip("bgMusic", "background.wav");
		defineAudioClip("jump", "jumpSound.wav");
		defineAudioClip("walk", "walking.mp3");
	}

	private JumpMan jumpman;
	private Kong kong;
	private Pauline pauline;
	private Heart lovePrevails;
	...
	private String[] climbUp = { "jc", "jc2" };
	private String[] paulineAnim = { "p1", "p", "p1", "p3" };
	...
	public void doFrame() {
				...
				if (kong != null) {
					...
					if (kong.x == 55) {
						...
						if (delayCount == 15) {
							...
							removeObjects(null, pauline.COLLISION_ID);
		...
		case GamePlay:
			...
			if ((delayCount == 15) && (jumpManState == JumpManState.MakeMe)) {
				...
				pauline = new Pauline(this, 270, 102);
				pauline.setAnim("pa");

				playAudio("song2", "bgMusic", true);
		...
		if (jumpman != null) {
			if (!jumpman.jumpmanDead) {
				...
				if (hasGyro()) {
					...
					if ((tiltY >= 20) && (!onLadder)) {
						if (!upKeyPressed) {
							upKeyPressed = true;
							jumpmanY = -4;
							startingY = (int) jumpman.y;
						}
					} else if ((tiltY >= -20) && (onLadder)) {
						jumpmanY = -3;
					} else if ((tiltY <= 20) && (onLadder)) {
						jumpmanY = 3;
					} else if (onLadder) {
	...
	private void winGame() {
		if ((jumpman.x < 380) && (jumpman.y < 205)) {
			winGame = true;

			jumpman.setGraphic(jumpman.TILE + "l");
			jumpman.xdir = 0;
			jumpman.ydir = 0;
			jumpman.x = 370;
			jumpman.y = 120;
			
			lovePrevails = new Heart(this, 320, 90);
		}
	}
